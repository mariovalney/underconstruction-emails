...
function wp_ajax_save_registry_callback() {
    ...
    check_ajax_referer( 'uc-add-registry-nonce', 'uc_nonce' );

    $response = array(
        'status' => 'error',
        'msg' => 'Ops... dados incorretos. Por favor, tente novamente.',
    );
        ...
        if ( count( $registries ) > 0 ) {
            ...
            if ( $registry ) {
                ...
                if ( $registry ) {
                    $response = array(
                        'status' => 'ok',
                        'msg' => 'E-mail salvo! Agora é só aguardar a novidade.',
                    );
                } else {
                    $response = array(
                        'status' => 'error',
                        'msg' => 'Não conseguimos salvar seu e-mail. Por favor, tente novamente.',
                    );
                }
